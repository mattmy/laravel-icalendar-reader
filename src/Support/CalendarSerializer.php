...
final class CalendarSerializer
{
    ...
    private function event(Event $event): array
    {
        return [
            ...
            'starts_at' => $this->dateTime($event->startsAt, $event->startIsDate), 'ends_at' => $this->dateTime($event->endsAt, $event->endIsDate),
            'start_is_date' => $event->startIsDate, 'end_is_date' => $event->endIsDate, 'start_is_floating' => $event->startIsFloating,
            'end_is_floating' => $event->endIsFloating, 'is_all_day' => $event->allDay, 'last_day' => $event->lastDay?->toDateString(),
            'duration' => $this->duration($event->duration), 'timestamp' => $event->timestamp?->toIso8601String(),
            'created_at' => $event->createdAt?->toIso8601String(), 'last_modified_at' => $event->lastModifiedAt?->toIso8601String(),
            'status' => $event->status, 'classification' => $event->classification, 'priority' => $event->priority,
            'recurrence_id' => $this->dateTime($event->recurrenceId, $event->recurrenceIdIsDate),
            'recurrence_id_is_date' => $event->recurrenceIdIsDate, 'recurrence_id_is_floating' => $event->recurrenceIdIsFloating,
            'sequence' => $event->sequence, 'url' => $event->url, 'organizer' => $event->organizer === null ? null : $this->organizer($event->organizer),
            ...
            'categories' => $event->categories->values()->all(), 'geo' => $event->geo, 'transparency' => $event->transparency,
            'comments' => $event->comments->values()->all(), 'contacts' => $event->contacts->values()->all(), 'resources' => $event->resources->values()->all(),
            'recurrence_rule' => $event->recurrenceRule?->toArray(), 'attachments' => $this->properties($event->attachments),
            'exception_dates' => $this->properties($event->exceptionDates), 'request_statuses' => $this->properties($event->requestStatuses),
            'related_to' => $this->properties($event->relatedTo), 'recurrence_dates' => $this->properties($event->recurrenceDates),
        ];
    }
    ...
    {
        return ['address' => $organizer->address, 'email' => $organizer->email, 'name' => $organizer->name, 'sent_by' => $organizer->sentBy, 'directory' => $organizer->directory, 'parameters' => $organizer->parameters()];
    }

    /** @return array<string, mixed> */
    private function attendee(Attendee $attendee): array
    {
        return ['address' => $attendee->address, 'email' => $attendee->email, 'name' => $attendee->name, 'role' => $attendee->role, 'status' => $attendee->status, 'rsvp' => $attendee->rsvp, 'type' => $attendee->type, 'delegated_from' => $attendee->delegatedFrom->values()->all(), 'delegated_to' => $attendee->delegatedTo->values()->all(), 'parameters' => $attendee->parameters()];
    }

    /** @return array<string, mixed> */
    private function alarm(Alarm $alarm): array
    {
        return ['action' => $alarm->action, 'trigger' => $alarm->trigger === null ? null : ['is_relative' => $alarm->trigger->isRelative(), 'is_absolute' => $alarm->trigger->isAbsolute(), 'duration' => $this->duration($alarm->trigger->duration()), 'date_time' => $alarm->trigger->dateTime()?->toIso8601String(), 'related_to' => $alarm->trigger->relatedTo()], 'description' => $alarm->description, 'summary' => $alarm->summary, 'attendees' => $alarm->attendees->map(fn (Attendee $attendee): array => $this->attendee($attendee))->all(), 'attachments' => $this->properties($alarm->attachments), 'repeat' => $alarm->repeat, 'duration' => $this->duration($alarm->duration)];
    }

    /** @return array<string, mixed> */
    private function component(Component $component): array
    {
        return ['name' => $component->name, 'properties' => $component->properties()->map(static fn (Property $property): array => $property->toArray())->all(), 'components' => $component->components()->map(fn (Component $child): array => $this->component($child))->all()];
    }
